    ...
    if cas_token_session_key in flask.session:
        ...
        if validate(flask.session[cas_token_session_key]):
            if 'CAS_AFTER_LOGIN_SESSION_URL' in flask.session:
                ...
                flask.session['CAS_AFTER_LOGIN'] = redirect_url
                current_app.logger.debug("lask.session['CAS_AFTER_LOGIN']:%s" % flask.session['CAS_AFTER_LOGIN'])
                current_app.logger.debug("CAS_AFTER_LOGIN_SESSION_URL:%s" % redirect_url)

            else:
                redirect_url = flask.url_for(
                    current_app.config['CAS_AFTER_LOGIN'])
                current_app.logger.debug("CAS_AFTER_LOGIN:%s" % redirect_url)
        else:
