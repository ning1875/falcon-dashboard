var app = angular.module('app', ['treeControl']);
app.controller('MyCtrl',function($scope, $http) {
	$http({
        ...
        $scope.groupsTreeModel = response.data
    ...
    $scope.onTreeLeafItemClickFunction = function(sel) {
          //console.log("sel.name",sel.name)
          fn_list_endpoints_fromtag(sel.name)
...
function fn_list_endpoints_fromtag(tag)
{
    ...
    $(".loading").show();
    $.getJSON("/api/tagendpoints", {tag: tag,  limit:limit, page:page, _r:Math.random()}, function(ret){
                $(".loading").hide();
                if (!ret.ok) {
                ...
                for (var hidx in ret.data) {
                    ...
                    var grafana_url ='https://grafana.xxxx.com/dashboard/db/openfalcon?orgId=1&var-allhosts='+h
                    var line_html = '<tr>'
                    + '<td><input type="checkbox" class="input shiftCheckbox" data-eid="'+ eid +'"  data-fullname="'+ h +'"></input></td>'
                    //+ '<td>' + h + '</td>'
                    + "<td><a href="+grafana_url+ ' target="_blank">'+ h + '</a></td>'
                    + '</tr>';
