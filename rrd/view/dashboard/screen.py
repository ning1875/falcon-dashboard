...
def dash_screens():
    ...
    top_screens = filter(lambda x: 'sys_base_screen_' not in x.name, top_screens)[:100]
    top_screens = sorted(top_screens, key=lambda x: x.name)

    return render_template("screen/index.html", **locals())


@app.route("/screensearch", methods=["GET"])
def dash_screensearch():
    name = request.args.get("search_str")
    top_screens = DashboardScreen.gets_by_name(name) or []
    return json.dumps(top_screens)
...
def dash_graph_edit(gid):
    ...
    if request.method == "POST":
        ...
        log.debug("Postcalledupdate %s %s %s %s %s %s %s %s " % (title, hosts, counters, screen_id,
                                                                 timespan, graph_type, method, position))
